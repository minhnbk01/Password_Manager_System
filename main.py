import sys
import user_service_db
import password_service_db
import user_service_file
import password_service_file
from security_utils import generate_strong_password
from virtual_keyboard import get_password_via_vk

user_service = user_service_file
password_service = password_service_file

def choose_storage_mode():
    global user_service, password_service
    print("\n=== CHỌN CHẾ ĐỘ LƯU TRỮ ===")
    print("1. Lưu bằng File cục bộ (.txt)")
    print("2. Lưu bằng Database (MySQL)")
    choice = input("Lựa chọn của bạn (1-2) [Mặc định là 1]: ")
    
    if choice == '2':
        user_service = user_service_db
        password_service = password_service_db
        print("[i] Đang sử dụng chế độ MySQL Database.")
    else:
        user_service = user_service_file
        password_service = password_service_file
        print("[i] Đang sử dụng chế độ File Text (.txt).")
...
def logged_in_menu(user_id):
    while True:
        ...
        if choice == '1':
            password_service.get_passwords(user_id)
        elif choice == '2':
            service = input("Nhập tên dịch vụ: ")
            password_service.get_password(user_id, service)
        elif choice == '3':
            service = input("Nhập tên dịch vụ: ")
            ...
            password_service.add_password(user_id, service, username, password)
        elif choice == '4':
            ...
            password_service.update_password(user_id, service, password)
        elif choice == '5':
            service = input("Nhập tên dịch vụ cần xóa: ")
            confirm = input(f"Bạn có chắc chắn muốn xóa '{service}'? (y/n): ")
            if confirm.lower() == 'y':
                password_service.delete_password(user_id, service)
        elif choice == '6':
            print("Đã đăng xuất.")
            break
        else:
...
def main_menu():
    ...
    while True:
        ...
        if choice == '1':
            ...
            user_service.register_user(username, password, phone, email)
        elif choice == '2':
            username = input("Nhập username: ")
            vk_choice = input("Sử dụng Bàn phím ảo cho mật khẩu để chống keylogger? (y/n): ")
            if vk_choice.lower() == 'y':
